...
if (isset($_POST["nama_toko"])) {
    ...
    // Tindakan jika $_POST["nama_toko"] tidak ada, misalnya beri nilai default atau tampilkan pesan kesalahan.
    $nama_toko = ""; // Atau tindakan lain sesuai kebutuhan Anda.
}

$querytampil = "SELECT nama_toko, bln_thn, SUM(jumlah_dropping) AS jumlah_dropping
FROM tb_dropping
WHERE nama_toko = '$nama_toko'
GROUP BY nama_toko, bln_thn
ORDER BY bln_thn";

$queryalpha = "select * from tb_alpha where id_alpha = 'A1'";
// $querybeta = "select * from tb_beta where id_beta = 'B1'";
// $querygamma = "select * from tb_gamma where id_gamma = 'G1'";
$querysum = "SELECT SUM(jumlah_dropping) FROM tb_dropping WHERE nama_toko = '$nama_toko'";
$pesan_error = "";
	...
	if ($_POST["submit"]="Ganti Alpha") {
		...
		if (($pesan_error === "") AND ($_POST["submit"]="Ganti Alpha")) {
			...
			if ($resultquery) {
				...
				header("Location: peramalan_semua_toko_dropping.php?pesan_sukses={$pesan_sukses}");
			}
			else {
...
<head>
	<meta charset="UTF-8">
	<title>Prediksi Dropping - Sari Roti</title>
...
<body>
	...
	<div class="content-container">
		<div class="container-fluid">
			...
			<div class="jumbotron">
				<h3 style="margin-bottom: 25px;"><?php echo $nama_toko;?></h3>
				<h4 style="margin-bottom: 25px;">A. Data Prediksi Dropping Semua Produk Pada Periode Berikutnya </h4>
				<div style="margin-bottom: 10px;">
					...
					<div class="row">
						...
						<div class="col-sm-8">
							<form action="peramalan_semua_toko_dropping.php" method="post" name="ubah_alpha" class="form-inline" onsubmit="return submitForm();">
								<div class="form-group">
									<input type="text" class="form-control" name="nilai_alpha" id="nilai_alpha" placeholder="Ganti Nilai Alpha">
								</div>

								<div class="form-group mx-sm-5">
									<select class="form-control" name="nama_toko" id="nama_toko">
										<option value="">-- Pilih Toko --</option> <!-- Opsi default -->
										<?php
										// Mengambil data nama_toko dari tabel tb_daftar_toko
										$queryToko = "SELECT nama_toko FROM tb_daftar_toko";
										$resultToko = mysqli_query($koneksi, $queryToko);

										// Loop untuk menambahkan pilihan dalam dropdown
										while ($rowToko = mysqli_fetch_assoc($resultToko)) {
											$selected = ($rowToko['nama_toko'] == $nama_toko) ? 'selected' : '';
											echo "<option value='" . $rowToko['nama_toko'] . "' $selected>" . $rowToko['nama_toko'] . "</option>";
										}
										?>
									</select>
								</div>
								<!-- <div class="form-group mx-sm-0" style="margin-bottom: 10px;">
								...
								<div class="form-group">
									<button type="submit" class="btn btn-primary" name="submit" value="Ganti Alpha">Proses</button>
								</div>
							</form>
						</div>
					</div>
				...
				//rumus rata2 abs_err MAD
						...
						<script>
							...
							var dataFirst = {
								label: "Aktual",
								data: [<?php
									$querydaktual = "SELECT nama_toko, bln_thn, SUM(jumlah_dropping) AS jumlah_dropping
									FROM tb_dropping
									WHERE nama_toko = '$nama_toko'
									GROUP BY nama_toko, bln_thn
									ORDER BY bln_thn";
								...
								var dataSecond = {
									...
									var speedData = {
										labels: [<?php
											$queryPeriode = "SELECT nama_toko, bln_thn, SUM(jumlah_dropping) AS jumlah_dropping
											FROM tb_dropping
											WHERE nama_toko = '$nama_toko'
											GROUP BY nama_toko, bln_thn
											ORDER BY bln_thn";
