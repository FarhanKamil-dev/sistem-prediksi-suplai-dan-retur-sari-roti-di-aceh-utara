...
if (isset($_POST["submit"])) {
	if ($_POST["submit"] == "Edit") {
		...
		$querytampilid = "SELECT * FROM tb_produk WHERE id ='$id'";
	...
	if ((!$pesan_error) && ($_POST["submit"] == "Update Data")) {
		...
		$queryupdate .= "kode_produk = '$kode_produk', nama_produk = '$nama_produk' ";
		$queryupdate .= "WHERE id = '$id'";

		$resultquery = mysqli_query($koneksi, $queryupdate);

		if ($resultquery) {
			$pesan_sukses = "Data produk berhasil diupdate!";
...
<body>
	...
	<div class="content-container">
		...
		<div class="container-fluid">
			...
			<div class="jumbotron">
				<h2 style="margin-bottom: 25px;">Edit Data Produk</h2>
				<?php
				if ($pesan_error !== "") {
				...
				?>
 <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="post" name="update_data">
        <div>
            <input type="hidden" name="id" value="<?php echo $id; ?>">
        </div>
        <div class="form-group">
            <label for="kode_produk">Kode produk</label>
            <input class="form-control" type="text" name="kode_produk" value="<?php echo $kode_produk; ?>" required>
        </div>
        <div class="form-group">
            <label for="nama_produk">Nama produk</label>
            <input class="form-control" type="text" name="nama_produk" value="<?php echo $nama_produk; ?>" required>
        </div>
