...
$querytampil = "SELECT p.kode_produk, r.bln_thn,  SUM(r.jumlah_retur) AS jumlah_retur 
FROM tb_retur1 r 
JOIN tb_produk p ON r.kode_produk = p.kode_produk 
WHERE r.nama_toko = 'zaskia'  AND p.kode_produk = '$nama_produk' 
GROUP BY r.nama_toko, r.bln_thn, p.kode_produk 
ORDER BY r.bln_thn";

$queryalpha = "select * from tb_alpha where id_alpha = 'A1'";
$querysum = "SELECT SUM(jumlah_retur)
FROM tb_retur1
WHERE nama_toko = 'zaskia' 
	...
	if ($_POST["submit"]="Ganti Alpha") {
		...
		if (($pesan_error === "") AND ($_POST["submit"]="Ganti Alpha")) {
			...
			if ($resultquery) {
				...
				header("Location: peramalan_zaskia.php?pesan_sukses={$pesan_sukses}");
			}
			else {
...
<body>
	...
	<div class="content-container">
		<div class="container-fluid">
			...
			<div class="jumbotron">
				<h2 style="margin-bottom: 25px;">TOKO ZASKIA</h2>
				<h3 style="margin-bottom: 25px;"><?php echo $nama_produk;?></h3>
				<h4 style="margin-bottom: 25px;">A. Data Prediksi Retur Periode Berikutnya </h4>
				<div style="margin-bottom: 10px;">
					...
					<div class="row">
						...
						<div class="col-sm-5">
							<form action="peramalan_zaskia.php" method="post" name="ubah_alpha" class="form-inline" onsubmit="return submitForm();">
								<div class="form-group">
									<input type="text" class="form-control" name="nilai_alpha" id="nilai_alpha" placeholder="Ganti Nilai Alpha" style="margin-bottom: 10px;"><br>
								</div>
					...
					<div class="col-sm-9">
						<table class="table table-hover table-bordered">
							...
							while ($row = mysqli_fetch_row($resultquery)) {
								// Ubah format tanggal dari yyyymmdd menjadi ddmmyyyy
								$tgl_database = $row[1];
								$tgl_diubah = date("dmY", strtotime($tgl_database));
								
								echo "<tr>";
								echo "<td>$tgl_diubah</td>";
								echo "<td>$row[2]</td>";
							}
				...
				//rumus rata2 abs_err MAD
						...
						<script>
							...
							var dataFirst = {
								label: "Aktual",
								data: [<?php
									$querydaktual = "SELECT p.kode_produk, r.bln_thn,  SUM(r.jumlah_retur) AS jumlah_retur 
									FROM tb_retur1 r 
									JOIN tb_produk p ON r.kode_produk = p.kode_produk 
									WHERE r.nama_toko = 'zaskia'  AND p.kode_produk = '$nama_produk' 
									GROUP BY r.nama_toko, r.bln_thn, p.kode_produk 
								...
								var dataSecond = {
									...
									var speedData = {
										labels: [<?php
											$queryPeriode = "SELECT p.kode_produk, r.bln_thn,  SUM(r.jumlah_retur) AS jumlah_retur 
											FROM tb_retur1 r 
											JOIN tb_produk p ON r.kode_produk = p.kode_produk 
											WHERE r.nama_toko = 'zaskia'  AND p.kode_produk = '$nama_produk' 
											GROUP BY r.nama_toko, r.bln_thn, p.kode_produk 
