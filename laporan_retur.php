...
if (!empty($start_date) && !empty($end_date)) {
    $querytampil .= " WHERE tb_retur1.bln_thn BETWEEN '$start_date' AND '$end_date'";
} elseif (!empty($start_date)) {
    $querytampil .= " WHERE tb_retur1.bln_thn >= '$start_date'";
} elseif (!empty($end_date)) {
    $querytampil .= " WHERE tb_retur1.bln_thn <= '$end_date'";
}
...
<body>
	...
	<div class="content-container">
		<div class="container-fluid">
			<div class="jumbotron">
				<h2 style="margin-bottom: 25px;">Laporan Data Retur Roti</h2>
				<?php
...
<div class="row">
    ...
    <div class="col-md-3">
        ...
        <button type="submit" class="btn btn-primary">Cari</button>
					...
					<a href="cetak_pdf_retur.php?search_nama_toko=<?php echo $search_nama_toko; ?>&search_nama_produk=<?php echo $search_nama_produk; ?>&start_date=<?php echo $start_date; ?>&end_date=<?php echo $end_date; ?>" target="_blank" class="btn btn-primary">Cetak PDF</a>
					...
					<div class="pagination">
						...
						if ($page > 1) {
							$pagelink .= "<a href='laporan_Retur.php?page=1&search_nama_toko=$search_nama_toko&search_nama_produk=$search_nama_produk&start_date=$start_date&end_date=$end_date'>First</a>";
							$prevpage = $page - 1;
							$pagelink .= "<a href='laporan_Retur.php?page=$prevpage&search_nama_toko=$search_nama_toko&search_nama_produk=$search_nama_produk&start_date=$start_date&end_date=$end_date'>Prev</a>";
						}
						
						for ($i = max(1, $page - 3); $i <= min($page + 3, $total_pages); $i++) {
							if ($i == $page) {
								$pagelink .= "<a class='active' href='laporan_Retur.php?page=$i&search_nama_toko=$search_nama_toko&search_nama_produk=$search_nama_produk&start_date=$start_date&end_date=$end_date'>$i</a>";
							} else {
								$pagelink .= "<a href='laporan_Retur.php?page=$i&search_nama_toko=$search_nama_toko&search_nama_produk=$search_nama_produk&start_date=$start_date&end_date=$end_date'>$i</a>";
							}
						}
						
						if ($page < $total_pages) {
							$nextpage = $page + 1;
							$pagelink .= "<a href='laporan_Retur.php?page=$nextpage&search_nama_toko=$search_nama_toko&search_nama_produk=$search_nama_produk&start_date=$start_date&end_date=$end_date'>Next</a>";
						}
						
						$pagelink .= "<a href='laporan_Retur.php?page=$total_pages&search_nama_toko=$search_nama_toko&search_nama_produk=$search_nama_produk&start_date=$start_date&end_date=$end_date'>Last</a>";
